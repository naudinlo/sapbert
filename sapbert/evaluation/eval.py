...
from sapbert.evaluation.utils import evaluate

from sapbert.src.data_loader import (
    DictionaryDataset,
    QueryDataset,
    QueryDataset_COMETA,
    QueryDataset_custom,
)
from sapbert.src.model_wrapper import Model_Wrapper
...
def parse_args():
    ...
    parser.add_argument(
        "--data_dir", type=str, required=True, help="data set to evaluate"
    )
    ...
    parser.add_argument(
        "--filter_composite",
        action="store_true",
        help="filter out composite mention queries",
    )
    parser.add_argument(
        "--filter_duplicate", action="store_true", help="filter out duplicate queries"
    )
    parser.add_argument(
        "--save_predictions", action="store_true", help="whether to save predictions"
    )
...
def main(args):
    ...
    if args.cometa:
        ...
        dataset = QueryDataset_custom(
            data_dir=args.data_dir, filter_duplicate=args.filter_duplicate
        )
