...
def predict_topk(
    model_wrapper, eval_dictionary, eval_queries, topk, cometa=False, agg_mode="cls"
):
    ...
    dict_dense_embeds = model_wrapper.embed_dense(
        names=dict_names, show_progress=True, agg_mode=agg_mode
    )
    ...
    for eval_query in tqdm(eval_queries, total=len(eval_queries)):
        ...
        for mention in mentions:
            ...
            np_candidates = [
                eval_dictionary[ind] for ind in candidate_idxs[0].tolist()
            ]  # .squeeze()
            dict_candidates = []
...
def predict_topk_fast(
    model_wrapper, eval_dictionary, eval_queries, topk, agg_mode="cls"
):
    """
    ...
    print("[start embedding dictionary...]")
    dict_dense_embeds = model_wrapper.embed_dense(
        names=dict_names, show_progress=True, batch_size=4096, agg_mode=agg_mode
    )
    ...
    for i in tqdm(
        np.arange(0, len(eval_queries), bs), total=len(eval_queries) // bs + 1
    ):
        ...
        candidate_idxs_batch = model_wrapper.retrieve_candidate_cuda(
            score_matrix=score_matrix, topk=topk, batch_size=bs, show_progress=False
        )
    ...
    for i in tqdm(range(len(eval_queries))):
        # print(candidate_idxs.shape)
        np_candidates = [
            eval_dictionary[ind] for ind in candidate_idxs[i].tolist()
        ]  # .squeeze()
        dict_candidates = []
        dict_mentions = []
        for np_candidate in np_candidates:
...
def evaluate(
    ...
    if medmentions or cometa:
        result = predict_topk_fast(
            model_wrapper, eval_dictionary, eval_queries, topk, agg_mode=agg_mode
        )
    else:
        result = predict_topk(
            model_wrapper, eval_dictionary, eval_queries, topk, agg_mode=agg_mode
